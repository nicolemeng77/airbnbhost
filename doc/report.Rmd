...
We noticed that there are several properties with abnormally high price which will potentially impact modelling fitting and sample tests. It is natural to remove them from the dataset. Furthermore, we have removed 1 listing that has over 20 bedrooms and 5 listings with more than 20 beds.     
...
```{r}
### LASSO ###
data = data[!(is.na(data$cleaning_fee)),]
random_list <- sample(nrow(data),nrow(data)*0.7)
train <- data[random_list,]
test <- data[-random_list,]

lasso_cv <- cv.glmnet(
  x = data.matrix(train[,c('host_is_superhost', 'host_listings_count',  'accommodates', 
                 'bathrooms',  'bedrooms',  'beds',  'cleaning_fee',  'minimum_nights',  'availability_365',
                 'number_of_reviews',  'instant_bookable',  'require_guest_phone_verification',  'hosted_years', 
                 'number_of_amenities',  'if_Manhattan',  'if_Brooklyn',  'if_Queens',  'if_Bronx',
                 'if_private', 'if_entire', 'if_shared')]), 
  y = train$price,
  alpha = 1
)
...
pred_lasso <- predict(lasso_reg, data.matrix(test[,c('host_is_superhost', 'host_listings_count',  'accommodates', 
                 'bathrooms',  'bedrooms',  'beds',  'cleaning_fee',  'minimum_nights',  'availability_365',
                 'number_of_reviews',  'instant_bookable',  'require_guest_phone_verification',  'hosted_years', 
                 'number_of_amenities',  'if_Manhattan',  'if_Brooklyn',  'if_Queens',  'if_Bronx', 
                 'if_private', 'if_entire', 'if_shared')]))
