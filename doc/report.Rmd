...
We have noticed that there are several properties with abnormally high price, however, after investigation we noticed that not all of these properties are outliers. Some of them truly exist. They are much more larger than normal apartments and contain more bedrooms and bathrooms. Thus, we will not treat them as outliers as they are reasonably priced. However, other properties that have fewer rooms but extremly high price are treated as outliers as they are not reasonable, and hence removed from the dataset.       
...
Firstly, we performed F-test to see if these two groups have same variance. The result below indicates that those two groups have same variance.     
```{r}
...
The following t test has a p-value of approximately zero, indicating a higher mean price for properties providing shampoo.    
```{r}
#Equal Variance
...
Furthermore, we have performed a two-way ANOVA test for shampoo and neighbourhood group. Both variables are significant with p-values less than 0.05. However, the interaction term is not significant in this case. 
...
From the above results, we conclude that providing shampoo or not is an important factor for pricing. Thus, we will be including the variable "have_shampoo" in the following models. Same procedures are done for several selected amenities and we found that having shampoo, kitchen, gym and elevator are related to higher prices, while parking is not a significant factor. Interestingly, we found that properties that allow pets tends to have lower prices.      
...
Since the model output, price, is a continuous variable, the first baseline model we have chosen is linear regression. We performed one-hot encoding on category variables. For example, neigbourhood_group variable, used to be a categorical variable with five levels, now expaned to five dummy variables if_Manhattan, if_Brooklym, etc.
...
Simple linear model achieved a test MSE of 48436.03 and R-square of 0.53. We discovered multicollinearity issues as Variance inflation factor for all vairables are larger than 1, with some of them over 10 indicating serious multicollinearity issue. Hence we performed feature selection using AIC statistics and removed three variables: 
host_identity_verified, bathrooms, have_laptop_workspac. However, we only observed a slight drop in test MSE and we decided to try regularized linear regression.
...
The test MSE of lasso regression model dropped significantly from 48436.03 to 18118.49. The R-square for lasso is 0.35, which is lower than that of linear regression since linear regression suffers from overfitting.

```{r}
a <- as.numeric(coef(lasso_reg, s = "lambda.1se"))
aa <- data.frame(rownames(coef(lasso_reg, s = "lambda.1se")), a)
aa <- aa[-1,]
colnames(aa) <- c('row', 'value')
ggplot(aa, aes(value, reorder(row, value))) +
  geom_point(show.legend = FALSE) +
  ggtitle("Influential Variables of Lasso Regression") +
  xlab("Coefficient") +
  ylab(NULL)
```

From the variable importance plot above, the top 3 variables that are determinant in pricing under Lasso model is Bathroom, 
...
For this random forest model, we achieved `r rf$r.squared` OOB R squared and `r rf$prediction.error` OOB MSE. It already outperformed linear regression, but we still needed to fine tune model parameters to further improve predicting accuracy. Hence we conducted hyperparameter tuning through full grid search on three parameters:

* mtry: The number of variables to randomly sample as candidates at each split. We picked a sequence evenly space around the default number 9.

* nodesize: Minimum number of samples within the terminal nodes. While smaller nodesize produces deep, complex trees and will introduce more variance, large nodesize produces shallow trees and will introduce more bias. Hence we picked a sequence of 3, 5 and 7 in hope to find the optimal balance of this bias-variance tradeoff situation.

* sampsize: The number of samples to train on. This is another bias-variance tradeoff situation as lower sampsize introduces bias and higher sampsize introduces variance. Hence we picked two numbers 0.632, the expected value of unique observations in the bootstrap sample, and 0.8, a reasonabally higher portion. 
...
Our tuning result shows that the optimal parameters to be num.trees = 500, mtry = 9 , min.node.size = 3 and sample.fraction = 0.8. Then we went ahead feed the tuned parameters into our random forest model and performed
visualization and prediction based on this final model.
...
From the variable importance plot, we can see that top 3 variables that are influential to the price under random forest model is room_type, longitude and accomodates,  !!!! MAY NEED TO UPDATE !!!!!!!. They are all in line with intuition.
...
We achieved final test MSE of `r MSE_RF` by random forest model, which has a ingnificant drop from linear regression and lasso models.


## XGBoost

Then we moved on to xgboost model in hope to further improve prediction power. Whereas random forests build an ensemble of deep independent trees, XGBoosts build an ensemble of shallow and weak successive trees with each tree learning and improving on the previous. When combined, these many weak successive trees produce a powerful “committee” that are often hard to beat with other algorithms.
...
We started with a basic XGBoost model with 1000 rounds and 5-fold cross validation. We picked objective function to be linear because of our continous dependent variable and early_stopping_rounds to be 10, which stops training if there is no improvement for ten consecutive trees.
...
From the plot above, we observed how training and test RMSE decreases as the number of trees grows. The training process stopped around 50 iterations when training RMSE stopped decreasing and the minumum Test RMSE we achieved is 74.93852 at around 40th iteration. 
...
We again tuned our xgoost model parameters through full grid search on five parameters:
* eta: learning rate
...
The tuning results are: eta = 0.01, max_depth = 7, min_child_weight = 1, 
sub_sampling = 0.65, colsample_bytree = 0.9.
...
We fed the optimal model parameters to test dataset and achieved test MSE to be 68.57523, which is slightly improved from the random forest model.
...
For XGBoost model, the top 3 influential variables are accommodates, bathrooms, availability_365. 
